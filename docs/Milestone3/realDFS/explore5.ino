...
void loop() {  
  ...
  // less than 750 off the line
    ...
    if((outerLeftVal > threshold) && (outerRightVal > threshold)){
      ...
      //front priority
      //check wall sensor
      if (detectNoFWall && visited[xfront][yfront] == 0) {
      ...
      } else if (detectNoLWall && visited[xleft][yleft] == 0) {
        //go left
        //update visited to reflect new current position and set the old current position to 1
        leftTurn();
        //add to stack 
        ...
        stack_push(robotX, robotY);
      } else if (detectNoRWall && visited[xright][yright] == 0) {
        rightTurn();
        //go right
        //update visited to reflect new current position and set the old current position to 1
        rightTurn();
