...
const int audioPin = A5;       //microphone
int startSignal = 0;          

const int treasure = A0;       //treasure
const int middleRightPin = A2; //right front line sensor 
const int middleLeftPin = A1;  //left front line sensor
const int intersectionPin =A3; //right and left outer line sensor


const int frontWall = 4;
const int leftWall = 3;
const int rightWall = 7;
const int treasure_detection_pin = 2;
...
void rightTurn() {
  ...
  while ((analogRead(intersectionPin)< threshold)){
    leftServo.write(leftServoAngle);
    rightServo.write(rightServoAngle);
    delay(delayTime);
  }
...
void leftTurn(){
  ...
  rightServo.write(rightServoAngle);
  delay(400);
  while ((analogRead(intersectionPin)  < threshold)){//here
    leftServo.write(leftServoAngle);
    rightServo.write(rightServoAngle);
    delay(delayTime);
  }  
...
/****************************************************************************audio setup*/
  ...
  // cli();  // UDRE interrupt slows this way down on arduino1.0
    while (digitalRead(pushButton)==LOW){ //((startSignal == 0) || (digitalRead(pushButton)==LOW)) {
