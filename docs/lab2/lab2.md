...
We used a different method to get the values for FFT than the Acoustic subteam did. On the Open Music Lab Library, we used the sketch they had (fft_adc_serial), rather than use analogRead we made it free-running. [Click here to see the fft sketch](http://wiki.openmusiclabs.com/wiki/Example)
